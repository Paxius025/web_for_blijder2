...
export default function LoginScreen() {
  ...
  useEffect(() => {
    // ✅ Wait for root navigation to be ready and avoid multiple attempts
    if (!rootNavigationState?.key || navigationAttempted.current) return;

    // ✅ Use queueMicrotask to defer navigation to next microtask
    queueMicrotask(() => {
  ...
  return (
    ...
    >
      ...
      <View className="flex-1 items-center justify-center px-6">
        ...
        <View className="w-full bg-white rounded-2xl p-7 shadow-lg">
          ...
          <View className="flex-row justify-end mb-5">
            ...
            <TouchableOpacity onPress={() => router.push("/register" as any)}>
              <Text className="text-sm text-[#0D6E4F] font-semibold">
                Register
              </Text>
            </TouchableOpacity>
          </View>
        ...
        <Text className="mt-6 text-xs text-white/30">
          © 2025 Seeing Eyes Inc.
        </Text>
      </View>
    </KeyboardAvoidingView>
  );
